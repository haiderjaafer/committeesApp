...
interface DynamicTableProps<T extends CommitteeDataTable> {
  ...
  onDataChange?: () => void; // ✅ NEW: Add callback prop for data refresh

}

interface CommitteeDeleteInfoTable {
  id: number;
...
export default function DynamicTable<T extends CommitteeDataTable>({
  ...
  excludeFields = [],
  pagination,
  onDataChange, // ✅ NEW: Receive refresh callback
  ...
  const [isAlertDialogDeleteAllContentsOpen, setIsAlertDialogDeleteAllContentsOpen] = useState(false);
  const [selectedCommitteeForDelete, setSelectedCommitteeForDelete] = useState<CommitteeDeleteInfoTable | null>(null);
  ...
  const handleDeleteClick = (e: React.MouseEvent, committee: CommitteeDeleteInfoTable) => {
    e.preventDefault();
    e.stopPropagation();
    
    console.log('Delete button clicked for committee:', committee);
    setSelectedCommitteeForDelete(committee);
    setIsAlertDialogDeleteAllContentsOpen(true);
  };

   // ✅ NEW: Updated handleDeleteSuccess with data refresh
  const handleDeleteSuccess = useCallback(() => {
    console.log('Committee deleted successfully');
    
    // Show success message
    toast.success('تم حذف اللجنة وجميع ملفاتها بنجاح!');
    
    // Reset state
    setSelectedCommitteeForDelete(null);
    setIsAlertDialogDeleteAllContentsOpen(false);
    
    // ✅ NEW: Trigger data refresh if callback provided
    if (onDataChange) {
      onDataChange();
    }
  }, [onDataChange]);
      ...
      .map((key) => {
        const columnDef: ColumnDef<T> = {
          ...
          cell: ({ row }) => {
            ...
            if (key === 'bookStatus') {
              const bgColorClass = getStatusBackgroundColor(valueStr);
              return (
            ...
            if (key === 'bookDate' || key === 'incomingDate') {
              return (
                <div className="text-right px-1 py-1 rounded-lg w-32">{valueStr}</div>
              );
            }
  ...
  return (
    ...
    const actionColumn: ColumnDef<T> = {
      ...
      cell: ({ row }) => (
        <div className="text-center flex items-center gap-1">
          {/* Update Button */}
          <Link href={`/updateCommittee/${row.original.id}`} title='تحديث اللجنة' className='border-2 border-amber-500 px-3 py-1.5 rounded-lg z-10'>
          ...
          <Button
            variant="ghost"
            className="cursor-pointer  border-amber-900 border-2 z-10 "
    ...
    <GrDocumentPdf   className="text-blue-600 "
    ...
    />
          ...
          {/*  Delete Committee Button - NO AlertDialog here */}
          <Button
            variant="ghost"
            className="cursor-pointer border-amber-900 border-2 z-10"
            onClick={(e) =>
              handleDeleteClick(e, {
                id: row.original.id,
                committeeNo: row.original.committeeNo,
                committeeTitle: row.original.committeeTitle,
                committeeBossName: row.original.committeeBossName,
              })
            }
            title="حذف اللجنة"
          >
  ...
  }, [data, headerMap, excludeFields, getStatusBackgroundColor, shouldTruncate, truncateText, handleDeleteClick]);
          ...
          <Dialog open={pdfDialogOpen} onOpenChange={setPdfDialogOpen}>
            <DialogContent className="sm:max-w-[600px] p-6 bg-gray-50" dir="rtl" aria-describedby="PDF_Dialog">
              ...
              {isLoadingPdfs ? (
                      ...
                      <CardContent className="pt-1">
                        <div className="space-y-2 text-right">
                          <div className="flex items-center gap-x-1">
                            ...
                            <span className="font-extrabold">{pdf.currentDate || 'غير متوفر'}</span>
                          </div>
                          <div className="flex items-center gap-x-1">
                ...
                <div className="text-center py-8 text-gray-500">لا توجد ملفات PDF</div>
              )}
            </DialogContent>
          </Dialog>
        </div>

        {/* ✅ DELETE COMMITTEE ALERT - OUTSIDE TABLE, AT ROOT LEVEL */}
        {selectedCommitteeForDelete && (
          <AlertDialogDeleteAllContents
            open={isAlertDialogDeleteAllContentsOpen}
            onOpenChange={setIsAlertDialogDeleteAllContentsOpen}
            committee={selectedCommitteeForDelete}
            onDeleteSuccess={handleDeleteSuccess}
          />
        )}

        {/* ✅ DELETE PDF ALERT - OUTSIDE TABLE */}
        {selectedPdf && (
          <AlertDialogDelete
            open={isDialogOpen}
  ...
  // Mobile view (keeping it simple for now)
  return (
